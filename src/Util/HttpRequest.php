...
use TianQi\PaySDK\Util\TianQiHttp\Http\Psr7\Uri;
use TianQi\PaySDK\Util\TianQiHttp\Http\Response;
use TianQi\PaySDK\Util\TianQiHttp\Stream\MemoryStream;
use TianQi\PaySDK\Util\TianQiHttp\Http\Request;
...
class HttpRequest
{
	...
	/**
	 * 发送内容，可以是字符串、数组（支持键值、TianQi\Util\TianQiHttp\Http\Psr7\UploadedFile，其中键值会作为html编码，文件则是上传）
	 * @var mixed
	 */
	public $content;
	...
	public function send($url = null, $requestBody = null, $method = 'GET')
	{
		...
		return TianQiHttp::send($request);
	}
